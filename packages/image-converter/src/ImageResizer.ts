...
export class ImageResizer extends Component {
	...
	throttledUpdateImageLink: (
		image: HTMLImageElement,
		newWidth: number,
		newHeight: number,
		currentHandle: string | null,
	) => void; // Throttled version of updateImageLink
	...
	attachView(markdownView: MarkdownView) {
		// Accept MarkdownView
	...
	private getCachedEditorMaxWidth(): number {
		if (this.cachedEditorMaxWidth === null) {
			...
			this.cachedEditorMaxWidth = (
				this.linkFormatter as any
			).getEditorMaxWidth();
		}
	...
	private registerEditorEvents() {
		...
		// 1. Hover Detection
		this.viewScope.registerDomEvent(
		...
		// 3. Register contextmenu event to clean up drag state on right-click
		this.viewScope.registerDomEvent(
			document as any,
			"contextmenu",
		...
		// 4. Register mousewheel event for resizing
		this.viewScope.registerDomEvent(
			this.markdownView.containerEl,
			"wheel",
			this.handleMouseWheel as any,
			{ passive: false },
		);
	}
	...
	private handleImageHover = (event: MouseEvent) => {
		...
		const activeContainer = this.activeImage?.matchParent(
		...
		if (
			...
			!target.hasClass("image-resize-handle")
		) {
		...
		if (
			target.instanceOf(HTMLImageElement) &&
			(this.isExternalLink(target.src) || isExcalidrawImage)
		) {
			if (
		...
		if (
			target.instanceOf(HTMLImageElement) &&
			!this.isExternalLink(target.src) &&
			!isExcalidrawImage
		) {
	...
	private handleEdgeDetection(
		...
		if (
			event.target &&
			(event.target as HTMLElement).hasClass("image-resize-handle")
		) {
			return;
		}
		...
		if (
			...
			if (
				(isNearTopEdge && isNearLeftEdge) ||
				(isNearBottomEdge && isNearRightEdge)
			) {
				imageTarget.style.cursor = "nwse-resize"; // Diagonal resize (top-left or bottom-right)
			} else if (
				(isNearTopEdge && isNearRightEdge) ||
				(isNearBottomEdge && isNearLeftEdge)
			) {
	...
	private cleanupHandles() {
		...
		if (handleContainer) {
			...
			handleContainer.parentNode?.insertBefore(
				this.activeImage,
				handleContainer,
			);
		...
		if (this.activeImage.hasClass("image-resize-border")) {
			// Check if this is an Excalidraw image and restore original attributes
			const filesource = this.activeImage.getAttribute("filesource");
			if (filesource && filesource.endsWith(".excalidraw.md")) {
				this.activeImage.setAttribute("draggable", "false");
				this.activeImage.style.pointerEvents = "";
				this.activeImage.style.userSelect = "";
			}
	...
	private handleMouseDown = (event: MouseEvent) => {
		// Ignore right-click and middle-click (only handle left-click)
		if (event.button !== 0) {
			// Clean up any existing drag state if right-clicking during a drag
			if (this.resizeState.isDragging || this.resizeState.isResizing) {
	...
	private startResize(
		event: MouseEvent,
		resizeTarget: HTMLElement | HTMLImageElement,
	) {
		this.resizeState.isResizing = true;
		this.activeImage =
			this.activeImage ||
			(resizeTarget.matchParent("img") as HTMLImageElement);
		...
		if (this.activeImage) {
			if (this.activeImage.hasClass("image-resize-border")) {
				...
				const container = this.activeImage.matchParent(
					".image-resize-container",
				);
				if (container) {
	...
	private handleMouseMove = (event: MouseEvent) => {
		...
		const runResizeCalc = () => {
			...
			if (
				this.activeImage &&
				this.activeImage.hasClass("image-resize-border")
			) {
			...
			if (this.currentHandle === "border") {
				...
				const isAspectFixed = true; // Always maintain aspect ratio
	...
	private handleMouseUp = () => {
		// Exit if scroll resizing is in progress
		if (this.resizeState.isScrolling) {
			return;
		}

		// If not resizing and not dragging, exit early
		if (!this.resizeState.isResizing && !this.resizeState.isDragging) {
			return;
		}

		// If no image is set but we have drag state, clean it up
		if (!this.activeImage) {
			this.cleanupDragState();
			return;
		}
		...
		const finalWidth =
			Number.isFinite(widthStyle) && widthStyle > 0
				? widthStyle
				: Math.round(this.initialWidth);
		const finalHeight =
			Number.isFinite(heightStyle) && heightStyle > 0
				? heightStyle
	...
	/**
	 * Handles the 'contextmenu' event (right-click). Cleans up drag state when right-clicking.
	 */
	private handleContextMenu = () => {
		// Clean up drag state if right-clicking during a drag operation
		if (this.resizeState.isDragging || this.resizeState.isResizing) {
			this.cleanupDragState();
		}
	};
	...
	/**
	 * Cleans up drag state without updating markdown link.
	 * Used when drag is cancelled (e.g., right-click).
	 */
	...
	private handleMouseWheel = (event: WheelEvent) => {
		...
		// Scroll resize is always enabled if main resize is enabled
		...
		const imageHash = `${notePath}|${imageName}`;
	...
	private getEndLineOfLink(
		...
		while (
			!lineContent.match(/\]\]|\)/) &&
			currentLine < editor.lastLine()
		) {
			//Added editor.lastLine() to avoid infinite loops
			currentLine++;
	...
	private async updateMarkdownLink(
		...
		const { editor } = this;
		const normalizedTargetName = this.isBase64Image(imageName)
			? imageName
			: this.getFilenameFromPath(imageName);
		...
		editor
			...
			.forEach((lineContent, line) => {
				...
				const matches = this.findAllMatches(lineContent).filter(
					(match) => {
				...
				matches.forEach((match) => {
					...
					const cachedWidth = undefined;
					const cachedHeight = undefined;
					...
					if (match.type === "md") {
						if (this.currentHandle === "border") {
							...
							widthParam =
								cachedWidth ??
								(match.existingWidth !== undefined
									? `${match.existingWidth}x`
									: "x");
							heightParam = `${Math.round(newHeight)}`;
							if (widthParam === "x")
								widthParam = `${this.initialWidth}x`;
						} else if (["e", "w"].includes(currentHandle || "")) {
							widthParam = `${Math.round(newWidth)}x`;
							heightParam =
								cachedHeight ??
								(match.existingHeight !== undefined
									? `${match.existingHeight}`
									: "");
							if (heightParam === "")
								heightParam = `${this.initialHeight}`;
						} else {
							widthParam = `${Math.round(newWidth)}x`;
							heightParam = `${Math.round(newHeight)}`;
						}
					...
					if (updatedContent) {
						...
						// Cursor positioning removed for minimal version
					}
				});
			});
	...
	private areEditorPositionsEqual(
		pos1: EditorPosition,
		pos2: EditorPosition,
	): boolean {
	...
	private isResizingPermitted(resizeType: "drag" | "scroll"): boolean {
		...
		const state = this.markdownView.getState();
		const isReadingMode = state.mode === "preview";
		// Reading mode resize is always enabled

		// Check specific resize type permissions
		if (resizeType === "drag") {
			return this.plugin.settings.isImageResizeEnbaled;
		}
