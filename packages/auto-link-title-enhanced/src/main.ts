...
interface DropFunction {
  (this: HTMLElement, ev: DragEvent, editor: Editor): void;
}
...
export default class AutoLinkTitle extends Plugin {
  ...
  async fetchUrlTitle(url: string): Promise<string> {
    try {
      ...
      // just the URL). On any failure we fall through to the rules below.
      if (this.settings.useLlm) {
        const page = await fetchPageContext(url, headers);
        if (page) {
          title = await getTitleViaLlm(url, page, {
